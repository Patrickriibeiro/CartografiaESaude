...
```{r decisoes}
adr_dir <- file.path("docs", "decisoes")
n_adr <- length(list.files(adr_dir, pattern = "^ADR-[0-9]{4}.*\\.md$"))
versoes_bancos <- bancos$versao[order(bancos$arquivo)]
n_col_lidas <- length(COLUNAS_SIVEP)
r1 <- function(ag, a) cmp$casos[cmp$regra == "R1_estrita_pdf" & cmp$agente == ag & cmp$ano == a]
r2 <- function(ag, a) cmp$casos[cmp$regra == "R2_vigilancia" & cmp$agente == ag & cmp$ano == a]
perda_r1_ini <- 1 - r1("sarscov2", ano_ini) / r2("sarscov2", ano_ini)
perda_r1_fim <- 1 - r1("sarscov2", ano_fim) / r2("sarscov2", ano_fim)
razao_min <- min(popd$razao_2024_censo) - 1; razao_max <- max(popd$razao_2024_censo) - 1
pares_queen <- contar_ligacoes(readRDS(file.path("resultados", "objetos", "pesos_queen.rds"))$neighbours) / 2
p_min_perm <- 1 / (nsim + 1)
nsim_habitual <- 999L   # o padrão do spdep, citado como comparação
p_min_habitual <- 1 / (nsim_habitual + 1)
limiar_fdr_min <- alfa / n_mun
n_scripts <- length(list.files(".", pattern = "^0[1-9]_.*\\.(R|qmd)$"))
```

Sete escolhas de método foram tomadas durante a implementação, cada uma registrada em um
documento de decisão (ADR, *architecture decision record*) em `docs/decisoes/`. Elas estão aqui
em forma de texto: o que foi decidido, por quê, e o que foi descartado.

## Extração dos dados (ADR-0001)

Os bancos anuais do SIVEP-Gripe são baixados diretamente do Portal de Dados Abertos do SUS, em
formato PARQUET, e cada arquivo é registrado com endereço, data e impressão digital SHA-256
antes de ser lido. A alternativa prevista na proposta original, o pacote `microdatasus`, foi
descartada porque ele não distribui o SIVEP-Gripe. O PARQUET foi preferido ao CSV porque lê só
as `r n_col_lidas` colunas usadas, das quase duzentas do banco, e já traz as datas como datas.
Como o banco do ano corrente é republicado toda semana, a versão usada é fixada e citada:
`r lista(unique(versoes_bancos))`.

## Critério de caso (ADR-0002)

Um caso de um agente é a ficha com classificação final daquele agente e, além disso, critério de
encerramento laboratorial ou o campo específico do vírus marcado. A leitura literal da proposta
original exigia sempre o campo específico; testada nos dados, ela perde `r fp(perda_r1_ini, 0)`
dos casos de SARS-CoV-2 de `r ano_ini` e `r fp(perda_r1_fim, 0)` dos de `r ano_fim`, porque o
preenchimento desse campo melhora ao longo dos anos, o que fabricaria uma queda artificial.
Cada ficha pertence a um único agente, o da classificação final; a detecção de um segundo vírus
é reportada, não contada duas vezes. Toda ficha do banco conta, sem filtrar a internação: o
banco é de SRAG notificada. As regras alternativas seguem no relatório como sensibilidade.

## Denominador populacional (ADR-0003)

O IBGE não publicou estimativa municipal para 2023, e as estimativas de 2024 e 2025 partem do
Censo 2022 corrigido pela subenumeração, correção maior nos municípios grandes: a estimativa de
2024 supera a contagem do Censo em todos os municípios, de `r fp(razao_min)` a `r fp(razao_max)`.
A decisão da autora foi usar dois denominadores, cada um com um uso: a população oficial do ano
no mapa e no LISA de cada ano (2023 interpolado nas datas de referência), e a estimativa de 2024
em todos os anos para as comparações entre anos, o que remove o degrau. As alternativas
descartadas foram repetir o Censo em 2023 e escalonar o Censo por um fator estadual único, que
é uma construção do projeto e não do IBGE. A seção sobre a população detalha as fórmulas.

## Moran global e LISA (ADR-0004)

A variável da análise espacial é a taxa suavizada por Bayes empírico, com a bruta como
sensibilidade; a vizinhança é Queen de primeira ordem sobre a malha oficial, `r pares_queen`
pares de vizinhos, com Rook como sensibilidade. Os testes usam `r fi(nsim)` permutações com
semente fixa, e não as `r nsim_habitual` habituais: com `r nsim_habitual` o menor p possível
(`r fd(p_min_habitual, 3)`) seria maior que o limiar mais exigente da correção para `r n_mun` testes
(`r fd(limiar_fdr_min, 5)`), e a correção seria
decidida pela resolução da simulação, não pelos dados; com `r fi(nsim)` o menor p é
`r fd(p_min_perm, 4)`. Os p-valores locais são corrigidos por Benjamini-Hochberg em cada mapa
(α = `r fd(alfa, 2)`), e o LISA sai em dois níveis, confirmado e indicativo, em vez de um só.
Os `r length(instaveis)` municípios com um único vizinho (`r lista(instaveis)`) mantêm a classe,
marcada como instável.

## Escopo e estrutura do pipeline (ADR-0005)

O pipeline são `r n_scripts` scripts numerados executados em ordem por um único comando, e não um
gerenciador de dependências como o pacote `targets`, que reexecuta só o que mudou: com `r n_mun`
polígonos e quatro anos, reexecutar tudo custa minutos, e quem avalia precisa ler o fluxo em
ordem sem aprender uma ferramenta. A escala é municipal, não por bairro (o "bairro" da proposta
original foi tratado como erro de digitação), e o quadrimestre é só descritivo; a análise
espacial roda por ano.

## Malha cartográfica (ADR-0006)

A malha é a oficial do IBGE, municípios de 2022, em resolução completa, lida direto do arquivo
compactado e registrada no manifesto. A alternativa, a malha simplificada distribuída pelo
pacote `geobr`, foi descartada porque a simplificação apaga trechos de fronteira entre
municípios vizinhos, e a vizinhança Queen mudaria com ela.

## Campos e chave de junção (ADR-0007)

Os nomes reais dos campos do SIVEP-Gripe, conferidos no dicionário oficial e nos quatro bancos,
são a fonte de verdade e ficam numa única constante; o preparo para se um deles faltar. A chave
de junção entre SIVEP, população e malha é o código IBGE de seis dígitos, sempre como texto: o
SIVEP grava seis dígitos e o IBGE sete, e uma junção com sete dígitos devolveria zero municípios
sem erro. Um teste confirma que os `r n_mun` municípios casam.

# Resultados

Todas as figuras seguem o mesmo código de cores: cada vírus tem uma cor única, usada nas séries,
nas dispersões e na rampa dos mapas de incidência, e os mapas de Moran local usam uma paleta
própria, sem nenhuma cor de vírus.

![Guia de cores: a cor de cada vírus, a rampa de incidência de cada um e as classes do Moran local](resultados/estatistica/guia_cores.png){#fig-guia-cores width=80%}
...
### Como a população de cada ano foi obtida

```{r populacao-metodo}
pop_r <- as.data.frame(arrow::read_parquet(file.path("dados", "processados", "populacao_rj.parquet")))
ex_cod <- "330330"   # Niterói: município médio, com casos em todos os anos
ex_nome <- nomes[[ex_cod]]
pex <- function(a) pop_r$populacao[pop_r$cod6 == ex_cod & pop_r$ano == a]
d0 <- as.Date("2022-08-01"); d1 <- as.Date("2024-07-01"); alvo <- as.Date("2023-07-01")
dias_ate_alvo <- as.numeric(alvo - d0); dias_total <- as.numeric(d1 - d0); w <- dias_ate_alvo / dias_total
p23_calc <- pex(2022) + w * (pex(2024) - pex(2022))
pi_ex <- ler_populacao_idade(ler_fontes()$ibge$populacao_idade$destino)
bebes22 <- pi_ex$populacao[pi_ex$cod6 == ex_cod & pi_ex$faixa == "<1"]
tot22 <- sum(pi_ex$populacao[pi_ex$cod6 == ex_cod])
bebes23 <- pex(2023) * bebes22 / tot22
ind_ex <- as.data.frame(arrow::read_parquet(file.path("dados", "processados", "indicadores_municipais.parquet")))
casos_ex <- ind_ex$casos[ind_ex$cod6 == ex_cod & ind_ex$agente == "vsr" & ind_ex$ano == 2023]
taxa_ex <- casos_ex / pex(2023) * 1e5
razao_ex <- pex(2024) / pex(2022)
fontes_pop <- ler_fontes()$ibge
tab_censo <- fontes_pop$populacao[[1]]$tabela; tab_idade <- fontes_pop$populacao_idade$tabela
```

O Censo é decenal e o estudo precisa de uma população por município para cada um dos
`r length(anos)` anos. Cada denominador tem uma origem diferente, e vale explicar as três
fórmulas com um exemplo, `r ex_nome`.

**`r ano_ini`: contagem do Censo.** A população residente em 1º de agosto de 2022, como o IBGE a
publicou (tabela `r tab_censo` do SIDRA): `r fi(pex(2022))` habitantes em `r ex_nome`. É uma contagem, sem
correção da parcela de pessoas que o recenseamento deixou de enumerar.

**2024 e 2025: estimativas do IBGE pelo método AiBi.** O IBGE não recenseia nesses anos; estima.
Para os municípios usa o *método de tendência do crescimento populacional*, conhecido como AiBi
e desenvolvido por Madeira e Simões em 1972 [@ibge2024estimativas]. A ideia: a população do
estado é projetada primeiro, pelo método das componentes demográficas (nascimentos, óbitos e
migração), e a de cada município acompanha a do estado por uma reta ajustada aos dois últimos
Censos:

$$P_i(t) = a_i \, P(t) + b_i, \qquad a_i = \frac{P_i(t_1) - P_i(t_0)}{P(t_1) - P(t_0)}, \qquad b_i = P_i(t_0) - a_i \, P(t_0)$$

em que $P_i$ é a população do município, $P$ a do estado, $t_0$ e $t_1$ são 1º de julho de 2010 e
de 2022, e $t$ é 1º de julho do ano estimado. O coeficiente $a_i$ diz que fração do crescimento
do estado cabe ao município; $b_i$ é a correção linear. Por construção, a soma dos municípios
reproduz o estado. Antes de ajustar a reta, o IBGE corrige as contagens dos dois Censos pela
subenumeração medida na Pesquisa de Pós-Enumeração, com ajuste maior nos municípios grandes.
É por isso que a estimativa de 2024 supera a contagem de 2022 em todos os municípios:
em `r ex_nome`, `r fi(pex(2024))` habitantes, `r fp(razao_ex - 1)` acima da contagem do Censo.

**2023: interpolação linear nas datas de referência.** O IBGE não publicou estimativa municipal
para 2023. O projeto traça a reta entre os dois números oficiais vizinhos, o Censo (1º de agosto de
2022) e a estimativa de 2024 (1º de julho de 2024), e a lê em 1º de julho de 2023:

$$P_{2023} = P_{2022} + w \,(P_{2024} - P_{2022}), \qquad w = \frac{\text{dias de 1º de agosto de 2022 a 1º de julho de 2023}}{\text{dias de 1º de agosto de 2022 a 1º de julho de 2024}} = \frac{`r dias_ate_alvo`}{`r dias_total`} = `r fd(w, 4)`$$

O peso não é meio, porque 1º de julho de 2023 está mais perto do Censo do que da estimativa.
Em `r ex_nome`: `r fi(pex(2022))` + `r fd(w, 4)` × (`r fi(pex(2024))` − `r fi(pex(2022))`) =
`r fi(p23_calc)` habitantes, o valor usado (`r fi(pex(2023))`, arredondado ao inteiro).

**Faixas etárias: a estrutura de 2022 em todos os anos.** O Censo 2022 é a única fonte com a
população municipal por idade (tabela `r tab_idade` do SIDRA). Para os outros anos, a proporção de cada faixa em
2022 é aplicada à população total do ano:

$$P_{a,\text{ano}} = P_{\text{ano}} \times \frac{P_{a,2022}}{P_{2022}}$$

Em `r ex_nome`, `r fi(bebes22)` dos `r fi(tot22)` habitantes de 2022 tinham menos de um ano
(`r fp(bebes22 / tot22, 2)`); aplicada à população de 2023, a proporção dá `r fi(bebes23)` bebês. A soma das
faixas é exatamente a população do ano, então a taxa bruta recalculada das faixas é a do estudo.

**A taxa.** Casos do ano divididos pela população do ano, por 100 mil habitantes. Em `r ex_nome`,
em 2023, foram `r fi(casos_ex)` casos de VSR para `r fi(pex(2023))` habitantes:
`r fi(casos_ex)` ÷ `r fi(pex(2023))` × 100 mil = `r fd(taxa_ex)` por 100 mil.
...
```{r regioes-ref}
trm <- utils::read.csv(file.path("resultados", "tabelas", "regioes_municipios.csv"), encoding = "UTF-8", stringsAsFactors = FALSE)
maior_reg <- trm[which.max(trm$populacao_2022), ]; menor_reg <- trm[which.min(trm$populacao_2022), ]
decreto_regiao <- "nº 7.508/2011"   # norma citada, não resultado: fica aqui pela regra de números no texto
```

**Por que também regiões de saúde.** O Decreto `r decreto_regiao` define a região de saúde como o espaço
geográfico contínuo de municípios limítrofes, delimitado por identidades culturais, econômicas e
sociais e por redes de comunicação e transporte compartilhadas, com a finalidade de integrar a
organização, o planejamento e a execução das ações de saúde [@brasil2011decreto]. É nessa escala
que o SUS planeja a rede hospitalar, regula leitos e pactua ações entre municípios, na Comissão
Intergestores Regional de cada região [@sesrj_regionalizacao]. Um resultado regional fala,
portanto, com quem decide onde abrir leitos ou concentrar a vacinação.

**Por que a análise principal continua municipal.** A região soma municípios muito diferentes: a
maior, `r maior_reg$regiao`, tem `r fi(maior_reg$populacao_2022)` habitantes, e a menor,
`r menor_reg$regiao`, `r fi(menor_reg$populacao_2022)`. Somar apaga contrastes internos, e o
resultado estatístico muda com o recorte, o problema da unidade de área modificável descrito
adiante. Por isso os agrupamentos são procurados nos `r n_mun` municípios, e a escala regional
entra como leitura de gestão.

![As regiões de saúde do Estado do Rio de Janeiro e seus municípios](resultados/mapas/regioes_saude_referencia.png){#fig-regioes-ref width=90%}

```{r tbl-regioes-municipios}
#| tbl-cap: "Regiões de saúde do Estado do Rio de Janeiro: municípios e população (Censo 2022)"
t15 <- data.frame(Região = trm$regiao, Municípios = trm$municipios, `População (2022)` = fi(trm$populacao_2022),
                  `Municípios que a compõem` = trm$lista_municipios, check.names = FALSE)
tab(t15)
```
...
![A mesma série com um painel por vírus, cada um na sua escala: o pico de SARS-CoV-2 de 2022 esconde, na escala comum, o tempo das ondas de influenza e VSR](resultados/estatistica/serie_semanal_estado_por_virus.png){#fig-serie-virus width=100%}
...
![Fichas de SRAG por semana e proporção ainda não encerrada, no último ano do estudo (dois painéis com o mesmo eixo do tempo)](resultados/estatistica/nao_encerrados_2025.png){#fig-nao-encerrados width=90%}

### Quanto os números mudam depois do fim do ano

```{r versoes}
vb <- utils::read.csv(file.path("resultados", "tabelas", "versoes_banco.csv"), encoding = "UTF-8", stringsAsFactors = FALSE)
te <- ler("resultados", "tabelas", "tempo_encerramento.csv")
v_fim <- vb[vb$ano == ano_fim, ]; ref_fim <- v_fim[v_fim$referencia, ]; mais_antiga <- v_fim[which.min(v_fim$dias_apos_fim_do_ano), ]
dif_max <- max(abs(unlist(lapply(AGENTES, function(ag) v_fim[[paste0("casos_", ag)]] - ref_fim[[paste0("casos_", ag)]]))))
v_ant <- vb[vb$ano == ano_fim - 1, ]
ant_igual <- nrow(v_ant) >= 2 && all(sapply(c("fichas", "encerradas", paste0("casos_", AGENTES)), function(cl) length(unique(v_ant[[cl]])) == 1))
reabertas <- sum(diff(v_fim$encerradas[order(v_fim$dias_apos_fim_do_ano)]) < 0)
pct_dif_max <- max(unlist(lapply(AGENTES, function(ag) abs(v_fim[[paste0("casos_", ag)]] - ref_fim[[paste0("casos_", ag)]]) / ref_fim[[paste0("casos_", ag)]])))
```

O banco do ano mais recente é republicado toda semana, e uma ficha pode ser acrescentada,
encerrada ou reclassificada depois do fim do ano. Para medir o efeito, foram comparadas
`r nrow(v_fim) - 1` versões anteriores do banco de `r ano_fim` publicadas no portal, a mais antiga
`r mais_antiga$dias_apos_fim_do_ano` dias depois do fim do ano epidemiológico, com a versão usada no
estudo (`r ref_fim$versao`), sempre pela mesma regra de caso. Entre elas, o número de fichas de
residentes passou de `r fi(mais_antiga$fichas)` para `r fi(ref_fim$fichas)`, e o de casos
confirmados de cada vírus variou no máximo `r dif_max` `r if (dif_max == 1) "caso" else "casos"`.
`r if (reabertas > 0) paste0("Em ", reabertas, " passagem de uma versão para a seguinte o número de fichas encerradas diminuiu: fichas já encerradas podem ser reabertas. ") else ""`Para
`r ano_fim - 1`, a versão publicada `r v_ant$dias_apos_fim_do_ano[!v_ant$referencia][1]` dias depois do fim
do ano `r if (ant_igual) "é idêntica à atual em fichas, encerramentos e casos dos três vírus" else "difere da atual"`.

```{r tbl-tempo-encerramento}
#| tbl-cap: "Tempo entre o início dos sintomas e o encerramento da ficha, por ano"
t16 <- data.frame(Ano = te$ano, `Dias até encerrar (mediana)` = te$dias_encerramento_mediana,
                  `Dias até encerrar (percentil 90)` = te$dias_encerramento_p90,
                  `Encerradas em até 30 dias` = fp(te$pct_encerradas_30d / 100, 0),
                  `Em até 90 dias` = fp(te$pct_encerradas_90d / 100, 0),
                  `Dias até digitar (mediana)` = te$dias_digitacao_mediana, check.names = FALSE)
tab(t16)
```

Dentro de cada banco, a mediana entre o início dos sintomas e o encerramento da ficha foi de
`r lista(paste0(te$dias_encerramento_mediana, " dias em ", te$ano))`. As versões mais antigas que o
portal ainda guarda para `r ano_fim` são de `r mais_antiga$dias_apos_fim_do_ano` dias depois do fim do
ano; o que muda nas primeiras semanas depois do fechamento não pôde ser medido. Daí em diante, a
maior diferença de casos entre versões, em qualquer vírus, é de `r fp(pct_dif_max, 2)` dos casos da
versão do estudo.

![Diferença, em número de casos, entre cada versão publicada do banco de 2025 e a versão usada no estudo](resultados/estatistica/versoes_banco_2025.png){#fig-versoes width=90%}
