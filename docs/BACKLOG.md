...
### Pedidos da analista (recebidos em 2026-09-26)

| ID | Título | Evidência | Modelo · Esforço | Aceite |
|---|---|---|---|---|
| CS-046 | **Paleta única por vírus e sem colisão com o LISA** — hoje o vermelho é SARS-CoV-2 nas séries e Alto-Alto no LISA, e o azul é influenza nas séries e Baixo-Baixo no LISA; `magma` em todos os mapas de incidência. Definir uma cor fixa por agente (séries, dispersões, faixas dos painéis, painel Shiny), uma escala sequencial de incidência por agente e uma paleta do LISA que não reutilize as cores dos vírus; registrar em `R/funcoes_mapas.R` como constantes únicas e no relatório (legenda-guia) | pedido da analista; `PALETA_LISA`, `funcoes_series.R:78`, `06_visualizacoes.R:114` | Opus · medium | 1 constante `CORES_AGENTE` usada em 100 % dos gráficos (grep); 0 cores compartilhadas entre vírus e classes LISA; contraste conferido (skill dataviz); mapas e painel regerados |
| CS-047 | **Revisão dos modelos de gráfico** — para cada figura do relatório (12 + painéis, séries, dispersões, barras com eixo duplo da maturação, gráfico de leitos), responder se a forma é a mais adequada ao que se quer mostrar, propor a troca quando não for (ex.: eixo duplo → dois painéis; dispersão com 92 pontos → identificar extremos) e aplicar; documento curto com a decisão por figura | pedido da analista | Fable · medium (crítica + execução, pequena) | `docs/revisao-graficos.md` com 1 linha por figura (mantida/trocada/por quê); figuras trocadas regeradas; teste de figuras do relatório verde |
| CS-048 | **Revisão dos casos entre versões do banco** — a analista observa que fichas mudam depois do "lançamento". Medir (a) dentro de cada banco: dias entre início dos sintomas e encerramento (`DT_ENCERRA`) e digitação (`DT_DIGITA`), proporção encerrada em 30/60/90 dias, por ano; (b) entre versões: função `comparar_versoes()` que, dado um snapshot anterior e um novo do mesmo ano, conta fichas do RJ acrescentadas, removidas e com `CLASSI_FIN`/agente alterado — hoje só existe a versão 14-09-2026 do banco de 2025 no portal (conferido em 2026-09-26), então (b) roda na 1ª atualização; seção no relatório | pedido da analista; ADR-0001 (banco vivo); CS-035 (não encerradas) | Opus · medium | tabela por ano (mediana e p90 de dias até encerrar; % em 30/60/90 d); função testada com 2 snapshots sintéticos; nota no relatório sobre o que muda entre versões |
| CS-049 | **População: explicação didática e fórmulas no texto** — o Censo não é anual: escrever, na proposta v2 §3.4 e no relatório, como cada denominador foi obtido — Censo 2022 (contagem), estimativas IBGE 2024/2025 (método do IBGE, com fonte), 2023 por interpolação linear nas datas de referência (fórmula com o peso 334/700), estrutura etária de 2022 aplicada aos outros anos (fórmula), com um exemplo numérico de um município | pedido da analista; ADR-0003; `docs/nota-metodologica-padronizacao.md` | Fable · medium (texto didático; método do IBGE conferido na nota técnica) | subseção "Como a população foi estimada" com 3 fórmulas e 1 exemplo numérico calculado pelo código; 0 números digitados; método do IBGE citado da nota metodológica oficial |
| CS-050 | **Mapa de referência das regiões de saúde** — mapa dos 92 municípios coloridos pela região, com contorno regional, nome da região e tabela região → municípios; parágrafo no texto sobre por que analisar também por região (escala de gestão do SUS: CIR, regulação de leitos) e por que a análise principal continua municipal (MAUP, CS-030) | pedido da analista; CS-030 | Opus · medium | 1 mapa em `resultados/mapas/regioes_saude_referencia.png` + tabela de 9 linhas no relatório; parágrafo com citação da SES-RJ |
| CS-051 | **Tabelas em formatação ABNT prontas para uso** — todas as tabelas de `resultados/tabelas/exportacao/` também num `.docx` (NBR 14724: título acima "Tabela N – ...", fonte abaixo, sem linhas verticais, traços só no topo, sob o cabeçalho e no rodapé, vírgula decimal), gerado pelo pipeline (etapa 07) com `flextable` + `officer` (CRAN; entram no `renv.lock`); a série semanal (6.270 linhas) entra resumida por mês, com a íntegra só em CSV | pedido da analista | Opus · medium | `resultados/tabelas/exportacao/tabelas_abnt.docx` com N tabelas = N CSVs; abre no Word; teste conta as tabelas do docx |
...
| CS-032 | **Série por semana epidemiológica** — 209 semanas (domingo a sábado, de `DT_SIN_PRI`), estado + 9 regiões × 3 agentes com zero explícito (6.270 linhas; estado = soma das regiões = 37.562); 10 gráficos + painel; campanhas de influenza 2022–2025 com fonte oficial em `config/fontes.yml`; pico de influenza 2022 antes da campanha (semana 1), nos outros anos 2 a 5 semanas depois | 2026-09-25 | `docs/release-history/cs-032-cs-035-series.md` · commit `798711f` |
...
| CS-036 | **Ano epidemiológico** — `limites_ano_epi()` (testada): 2022 02/01–31/12, 2023 01/01–30/12, 2024 31/12/2023–28/12, 2025 29/12/2024–03/01/2026 (53 semanas; a semana 53 soma 28 casos, 0,4 %); meio do ano a no máximo 2 dias de 1º de julho, exceto o Censo 2022 (29 dias); proposta §3.1 e §3.3 corrigidas (a §3.3 descrevia substituição por DT_NOTIFIC que o código não faz); nota calculada no relatório; convenção no ADR-0003. Suíte 581/581 em 18 arquivos | 2026-09-25 | `docs/release-history/cs-036-ano-epidemiologico.md` · commit `b9dfe7b` |
...
| CS-029 | **Ecossistema `.claude/`** — `CLAUDE.md` (127 linhas: exigências do dono, invariantes com cicatriz, classes de documento, fechamento de item, hooks, armadilhas), hook `SessionStart` de contexto e hook que nega commit de código sem release-history; 10/10 casos testados em repositório descartável | 2026-09-25 | `docs/release-history/cs-029-ecossistema-claude.md` · commit `3125536` |
| CS-045 | **Hook de append-only** — `guarda-append-only.ps1` nega Edit/MultiEdit/Write destrutivo em ADR e release-history já commitados (trecho antigo tem de sobreviver; Write tem de começar com o conteúdo atual); 14/14 casos em repositório descartável + 1 no real; limites (inserção no meio passa; Bash escapa) no CLAUDE.md §5 | 2026-09-25 | `docs/release-history/cs-045-hook-append-only.md` · commit `d666f60` |
| CS-033 | **Padronização por idade (OE9)** — método direto, 11 faixas, padrão RJ 2022 (SIDRA 9514, no manifesto), estrutura de 2022 aplicada aos outros anos (contrato: bruta recalculada das faixas == incid_100k); `incid_pad_100k` nos 3 agentes; correlação de postos bruta × padronizada 0,989–0,999, razão mediana 0,93–1,01 (VSR < 1 nos 4 anos: 73 % dos municípios com mais bebês que o padrão); extremos com 1 caso; gráfico; nota metodológica; seção no relatório. Suíte 657/657 em 21 arquivos | 2026-09-25 | `docs/release-history/cs-033-padronizacao-idade.md` · commit `a035515` |
| CS-028 | **Seções vazias do PDF** — Introdução (1.1 Contextualização, 1.2 Problematização) e Conclusão (6) escritas no relatório com todos os números calculados; conclusão responde à pergunta e a H1–H4 uma a uma (H1 6/12 fraca; H2 só VSR 2024; H3 na série, não nos mapas; H4 descritiva); 4 referências novas conferidas na fonte; proposta v2 §1.1 sem [REVISAR] e §6 preenchida. Apêndice A é texto pessoal da autora e não foi redigido. Suíte 661/661 em 21 arquivos | 2026-09-25 | `docs/release-history/cs-028-secoes-do-relatorio.md` · commit `b71d4b5` |
...
| CS-026 | **Repositório público** — histórico reescrito uma 2ª vez para trocar o e-mail pessoal dos 65 commits pelo endereço noreply do GitHub (hashes citados em documentos e mensagens trocados; mapa antigo → novo no release-history); ensaio num clone idêntico ao resultado real; e-mail `noreply` configurado no repositório; repositório público desde 2026-09-25, acesso anônimo HTTP 200, licença MIT reconhecida, `CITATION.cff` no repositório | 2026-09-25 | `docs/release-history/cs-026-publicacao.md` · commit `a5d5453` |
| CS-025 | **CI no GitHub Actions** — 1ª execução verde depois que o repositório ficou público: run 36200764263, 1 min 55 s, 553 expectativas em 21 arquivos, 6 pulados (dados reais fora da CI); badge do README ativo | 2026-09-25 | `docs/release-history/cs-025-integracao-continua.md` · commit `a1796db` |
