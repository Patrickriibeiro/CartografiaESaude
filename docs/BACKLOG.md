...
|---|---|---|---|
| D-01 | Quem é o dono do repositório GitHub público (item 4.5 do PDF): Patrick, Gabrielle ou ambos? Qual licença (MIT para código, CC-BY para relatório)? | Ambos como autores; MIT + CC-BY 4.0 | **Decidida 2026-09-25 (autora):** Gabrielle e Patrick como autores; MIT para o código, CC-BY 4.0 para o relatório; o PDF da proposta sai do repositório antes de publicar (ver CS-026: ele está no histórico desde `b66ae13`) | CS-025 | **CI no GitHub Actions — falta só a 1ª execução** — `.github/workflows/testes.yml` escrito e validado (Ubuntu 24.04, R 4.6.1, binários do Posit Package Manager, `renv` com cache, suíte com resumo de pulados); suíte portável (`url_arquivo()`). **Aceite pendente: workflow verde e < 10 min na 1ª execução, que depende do CS-026** | `docs/release-history/cs-025-integracao-continua.md` | Opus · low | 1ª execução verde, < 10 min, 6 testes pulados |
| CS-026 | **Repositório público — preparado, publicação aguarda o dono** — feito: `LICENSE` (MIT), `LICENSE-CONTEUDO.md` (CC-BY 4.0 + dados de terceiros), `CITATION.cff` (validado), README com seção de licença. **Falta decidir e executar:** (1) conta/organização e nome do repositório; (2) como tirar o PDF da proposta, que está só no commit `b66ae13` de 36 (reescrever o histórico e atualizar os hashes citados nos docs, ou publicar um snapshot novo); (3) privado primeiro para ver a CI (CS-025) e depois público | D-01 decidida; `gh` autenticado em `Patrickriibeiro` com escopo `repo` e `workflow` | Opus · low | repositório criado, PDF ausente de todo o histórico público, CI verde, `CITATION.cff` reconhecido pelo GitHub |
